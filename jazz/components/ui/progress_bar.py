...
from ...utils import Texture, Vec2, clamp, map_range
...
class ProgressBar(Sprite):
    ...
    def __init__(self, value: float = 0, max_value: float = 100, **kwargs) -> None:
        """Initializes the ProgressBar component.

        Args:
            value (float): Current numeric value. The bar is drawn clamped to 0..max_value.
    ...
    def update_bar(self) -> None:
        ...
        The fill is clamped to the bar: a value above max_value draws a full bar,
        and a value at or below 0, or a max_value at or below 0, draws an empty one.
        """
        ...
        if self.value > 0 and self.max_value > 0:
            fill_width = map_range(
                clamp(self.value, 0, self.max_value),
                0,
                self.max_value,
                0,
                w - self.line_width * 2,
    ...
    def update_value(self, value: float) -> None:
        ...
        Args:
            value (float): New current progress value.
        """
        self.value = value
        self._value_dirty = True
    ...
    def update_max_value(self, max_value: float) -> None:
        ...
        Args:
            max_value (float): New maximum limit value.
        """
        self.max_value = max_value
        self._value_dirty = True
