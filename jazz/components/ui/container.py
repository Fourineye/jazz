"""Layout container UI components: UIContainer, VBox, and HBox."""
...
class UIContainer(Sprite):
    ...
    def __init__(self, name: str = "UIContainer", **kwargs) -> None:
        ...
        # A bg_color background is generated by layout(); don't fall back to the "default" texture
    ...
    def remove_child(self, obj: BaseObject, kill: bool = True) -> None:
    ...
    def texture(self) -> Texture | Image | str | None:
        """Texture | Image | str | None: Gets the active Texture or Image asset, or None if no background."""
        return self._texture

    @texture.setter
    def texture(self, new_texture: str | Texture | Image | Surface | None) -> None:
        """Sets the texture asset, preserving the logical size of a bg_color container.

        Args:
            new_texture (str | Texture | Image | Surface | None): Asset key, source
                image surface, or None to clear the background.
        """
    ...
    def render(self, offset: Vec2) -> None:
        ...
        if not self._has_background or not isinstance(texture, (Texture, Image)):
            return

        if self.bg_color is not None and hasattr(self, "_kwargs"):
            # Styled backgrounds include shadow padding around the container area
            shadow_offset = self._kwargs.get("shadow_offset", (2, 4))
            shadow_blur = self._kwargs.get("shadow_blur", 4)
            pad = Vec2(
